...
// BotConfig holds configuration for the content bot (cmd/bot).
// The bot is a pure HTTP client: it registers/logs in bot accounts against the
// running API and publishes posts whose content is generated by Gemini via the
// AI Studio API. GeminiAPIKey is required only by cmd/bot — the API server
// ignores this section entirely, so it is not part of Config.Validate.
