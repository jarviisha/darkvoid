...
- [ ] T010 [P] [US1] Implement in `internal/feature/feed/dispatcher.go`: `NewEventDispatcher` accepts `ScorerConfig` (store the precomputed write-time constant); `EmitPostCreated` packs it with `createdAt`. T008 green.
- [ ] T011 [P] [US1] Implement in `internal/feature/feed/refresher.go`: `NewPreparedTimelineRefresher` accepts a `feed.Ranker`; `refreshOne` builds followingSet from authorIDs, calls `RankPosts`, packs each score with that post's CreatedAt, writes via `SetPostsBatch`. T009 green.
