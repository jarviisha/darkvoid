...
- [X] T020 Manual verification per quickstart.md — executed pre-merge in the PR against an ephemeral stack (postgres:16-alpine on 15432 + redis:alpine on 16379 + API on 18080 with isolated env, zero contact with the shared dev stack). Results: fan-out post appeared immediately with served score exactly 30.0; packed ZSET score decoded to bucket 30000 / correct createdAt second; only feed:tl:v2 keys written; DEL + lazy refresh rebuilt the timeline including a pre-follow post with decayed local scores (29.525/29.399 — 0.001 resolution visible); 27 posts paginated 20+7 with zero duplicates/gaps. Observed (design-consistent): posts created before a follow only appear after a refresh — fan-out reaches existing followers only; P3 follow-event trigger will close this
