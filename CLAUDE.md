...
Container modes, release bundles, environment boundaries and upgrade instructions: [docs/container-deployment.md](docs/container-deployment.md).
...
- `CODOHUE_ADMIN_TOKEN` + `CODOHUE_NAMESPACE_KEY` — the two credentials provisioning cannot invent, both checked at boot by `validateCodohue`. The namespace key is **ours to choose and send**: provisioning puts it in `provision_api_key`, where it used to be minted server-side and read back out of the response, so a retried call produced a second key and the ends disagreed about which was current. `CODOHUE_ADMIN_TOKEN` is an admin-plane service token sent as a bearer; it replaces `CODOHUE_ADMIN_KEY`/`CODOHUE_API_KEY` and the `POST /api/v1/auth/sessions` login behind them, which Codohue v0.12.0 retired — measured against v0.12.1, that login answers 403. Provisioning is still hand-rolled rather than `sdk/go/admin`: as of SDK v0.7.0 that package's request type carries `action_weights`, `alpha` and `dense_distance` only, so adopting it would drop `lambda`, `gamma`, `max_results`, `seen_items_days` and the three trending knobs to server defaults — pinned by `TestProvisionNamespaceConfig_SendsTheKnobsTheAdminSDKOmits`.
- `CODOHUE_ENABLED` — **Codohue is never started from this repo.** It has no containers, no Qdrant and no schema here; `compose.dev.yml` carries no `codohue` profile and there is no `make docker-up-codohue`. Deploy it from its own compose project and point `CODOHUE_BASE_URL` / `CODOHUE_ADMIN_URL` at it — this project only ever *reaches* it, over HTTP and over the events Redis Stream. Because `.env`'s URLs are written from the host's point of view and `localhost` inside a container is the container, the `app`, `seed` and `ctl` services override both, defaulting to `host.docker.internal`; `CODOHUE_DOCKER_BASE_URL` / `CODOHUE_DOCKER_ADMIN_URL` override that in turn. When false, feed uses local scoring only; CF recommender is off. When true, the client is wired whether or not Codohue answers at startup: an unreachable Codohue is already handled per call (the feed falls back to local scoring, ingest logs and moves on), so gating the wiring on a boot probe only meant a brief outage disabled the feature for the whole process. The probe now just classifies, `GET /health` reports `codohue: off|active|degraded`, and a background monitor re-probes every 2 minutes for the life of the process — not only after a failed boot probe, or `/health` would assert `active` through an outage that started a minute after boot. Nothing is rewired at runtime, so no service field is written once the server is serving. `pkg/codohue` carries a circuit breaker over the HTTP surface: 3 consecutive availability failures open it for 30s, so an outage costs one timeout rather than 2-3s added to every feed request. Most 4xx count as *available* — the service answered, and opening the circuit because one call site sent something malformed would disable the integration for everyone. **401 and 403 are the exception**: a rejected credential fails every caller identically until an operator changes something, so it opens the circuit like an outage. `Ping` probes the namespace (a `Trending` read), not the SDK's unauthenticated `/ping`, because `/ping` answers 200 from a deployment that rejects every real call — that is how a deleted namespace left `/health` reporting `active` for six days while nothing worked. It bypasses the gate (a health probe answered from a cached "circuit open" measures nothing) but reports its outcome, so a successful probe closes the circuit at once. `/health` reads the breaker as well as the last probe: the breaker learns of an outage from real traffic within a few requests, where the probe would not notice until its next tick, so an open circuit overrides a stale `active`.
- **Behavior events carry `namespace_generation`.** It comes from the provisioning response, is held on `Application.codohueGeneration` until the client is built, and is stamped by `newEventProducer`. Codohue accepts an unstamped envelope only for a generation-1 namespace with its legacy gate open, so after that gate closes or the namespace is recreated, an unstamped event is dropped when the consumer reads it — silently, because publishing is fire-and-forget. Zero means nothing was reported (`darkvoidctl` does not provision; provisioning may have failed) and publishes unstamped, the behaviour from before the field existed.
...
- **Hostname pinning under `compose.codohue.yml`.** A container on two compose networks sees both projects' service names, so bare `postgres` and `redis` each resolve to two servers with no guarantee which wins — measured on the dev box, `redis` from the app container resolved to Codohue's instance. The dev/prod infrastructure overlays give our datastores network-unique aliases (`darkvoid-postgres`, `darkvoid-redis`) and addresses Codohue's Redis by container name via `CODOHUE_STACK`. Using a bare service name for either is the failure that hides: the app boots, the health check passes, and the cache or the event stream is quietly pointed at the wrong server.
...
- `TRUSTED_PROXY_CIDRS` — comma-separated CIDRs whose peers may set `X-Forwarded-For`/`X-Real-IP`. `appmiddleware.TrustedRealIP` rewrites `RemoteAddr` only for peers inside them, so an empty list means forwarding headers are ignored entirely — correct for local development, where the client *is* the peer, and wrong behind a proxy, where the only peer is the proxy. Every request then rate-limits into one `httprate.LimitByIP` bucket (100 req/min shared by the whole deployment) and the access log records the proxy as the caller. `compose.prod.yml` therefore defaults it to loopback plus the private ranges a bridge network can occupy, since the app publishes on `127.0.0.1` and is reached through a host proxy; narrow it to the proxy's own address when `SERVER_BIND` is opened further, or a client arriving directly from one of those ranges can forge its own address.
- `STORAGE_PROVIDER` — `local` or `s3`; any other value fails startup instead of silently discarding uploads. `local` serves `/static/*` from `STORAGE_LOCAL_DIR` and is refused outright when `ENVIRONMENT=production`, because a container-local directory is not shared between instances and a rolling replacement loses whatever was uploaded to the one going away. That rule lives in `pkg/config.validateStorage` rather than as a `${VAR:?}` in `compose.prod.yml`: a required variable there is evaluated by *every* Compose command, so an unset one takes `dv ps` and `dv logs` down with the deploy, and it fires on the non-production deployments that share the file rather than on the one it is meant to protect. The static route refuses any dot-prefixed path segment (`middleware.HiddenFileGuard`) — no media key has one, and the local provider's writability probe is a dotfile inside that same served directory.
  Switching an existing deployment from `local` to `s3` is a **data migration, not a variable change**: `post.post_media`, `post.comment_media` and `usr.users.avatar_key` hold bare keys, and the provider resolves a key to a URL at read time, so every image and video already posted 404s the moment the provider changes. Run `scripts/storage/migrate-local-to-s3.sh --source <uploads dir> --apply` first (dry run by default; it syncs keys unchanged into `STORAGE_S3_BUCKET`), then change the variable.
